...
namespace WebViewTranslate.Translator
{
    ...
    public class CopyPasteTranslator
    {
        ...
        public List<CopyBlock> BuildBlocks(int maxBlockSize, string sourceLanguage, int startIndex)
        {
            ...
            for (var index = startIndex; index < _paragraphs.Count; index++)
            {
                ...
                {
                    result.Add(new CopyBlock { TargetText = input.ToString().Trim(), Paragraphs = paragraphs });
                    input.Clear();
        ...
        public List<string> GetTranslationResult(string targetLanguage, string target, CopyBlock block)
        {
